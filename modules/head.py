...
from .block import DFL, Proto, GSConv
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        # self.cv2 = nn.ModuleList(
        #     nn.Sequential(GSConv(x, c2, 3), GSConv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch)

        # self.cv3 = nn.ModuleList(
        #     nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
        self.cv3 = nn.ModuleList(
            nn.Sequential(GSConv(x, c3, 3), GSConv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
...
class Segment(Detect):
    ...
    def __init__(self, nc=80, nm=32, npr=256, ch=()):
        ...
        #     nn.Sequential(GSConv(x, c4, 3), GSConv(c4, c4, 3), nn.Conv2d(c4, self.nm, 1)) for x in ch)
