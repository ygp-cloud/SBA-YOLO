...
__all__ = (
    'Conv', 'LightConv', 'DWConv', 'DWConvTranspose2d', 'ConvTranspose', 'Focus', 'GhostConv', 'Concat', 'RepConv',

    'SimFusion_4in', 'SimFusion_3in', 'IFM', 'InjectionMultiSum_Auto_pool', 'PyramidPoolAgg', 'AdvPoolFusion',
    'TopBasicLayer',
    'BasicRFB',
...
class RepConv(nn.Module):
    """RepConv is a basic rep-style block, including trai
    ning and deploy status
...
class SimFusion_3in(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.cv_fuse(torch.cat((x0, x1, x2), dim=1))
        return out
...
class InjectionMultiSum_Auto_pool(nn.Module):
    ...
    def forward(self, x):
        '''
        x_g: global features->(g_B, g_C, g_H, g_W)
        x_l: local features->(B, C, H, W)
        '''
        x_l, x_g = x
        B, C, H, W = x_l.shape
...
class PyramidPoolAgg(nn.Module):
    ...
    def forward(self, inputs):
        ...
        output = self.conv(torch.cat(out, dim=1))
        return output
...
class MAM(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        y = self.avgpool(x).view(b, c)
        x_channel_att = self.channel_attention(y).view(b, c, 1, 1)
        x = x * x_channel_att
